// Minimal SHA-256 + HMAC-SHA256 (public-domain style implementation).
#ifndef ORI_SHA256_H
#define ORI_SHA256_H
#include <stdint.h>
#include <string.h>

typedef struct { uint32_t s[8]; uint64_t len; uint8_t buf[64]; int n; } Sha256;
...
static void sha256_update(Sha256* c, const uint8_t* p, size_t len){
    c->len+=len;
    while(len){
...
static void sha256(const uint8_t* p, size_t len, uint8_t out[32]){
    Sha256 c; sha256_init(&c); sha256_update(&c,p,len); sha256_final(&c,out);
}
